...
public abstract class AbstractSearchEngine implements RestrictedSearchEngine<String,URL,URL>, AutoCloseable {
    private final static String TAG=AbstractSearchEngine.class.getName();
	...
	Ii<Option<URL>, List<String>> queryRemainder=format(pDisjunctions);
	if(pDisjunctions.size()==queryRemainder.snd().size() || pDisjunctions.size()==0 || !queryRemainder.fst().hasVal()) {
	    return NULL_QUERY;
	}
	return new HttpQuery<String>(uniqueName, queryRemainder.fst().val());
    }
     ...
     */
    protected abstract Option<URL> uri() throws MalformedURLException;
     ...
     * @throws URISyntaxException 
     */
    protected Ii<Option<URL>, List<String>> format(List<String> pDisjunctions) 
	throws UnsupportedEncodingException, MalformedURLException {
	Log.i(TAG, "format");
	reset();
	int i=0;
	...
	return new Ii<Option<URL>, List<String>>(uri(), Collections.unmodifiableList(pDisjunctions.subList(i, pDisjunctions.size())));
    }
}
