...
The report generator divides all queries generated into batches and processes each batch in turn. A smaller value for NUM_BATCHES argument in the example below requires more heap, but should ensure that the report is created more quickly. Ideally you should set the jvm to use as large a heap as possible and use a small value for NUM_BATCHES. If the report generator crashes due to lack of memory double NUM_BATCHES and try again. Any batches completed before the crash will be skipped after the restart and won't have to be repeated.

The output is saved in the poems subdirectory of current directory's poems. Just open poems/poems.html in a browser ensuring that you have first disabled restrictions on local file browsing. In chrome this can be done by running the browser with the following command:
chrome --allow-file-access-from-files
Firefox has an option that can be set by entering about:config in the address bar. Look for the security.fileuri.strict_origin_policy entry option and set this to false

The report also accesses files from the documents subdirectory. This directory should contain the output from running MainDownloader class (as described above) so this directory also needs to be available.

java -ea -cp lib/jackson-annotations-2.4.2.jar:lib/jackson-core-2.4.2.jar:lib/jackson-databind-2.4.2.jar:lib/lucene-core-4.10.1.jar:lib/lucene-analyzers-common-4.10.1.jar:lib/lucene-analyzers-stempel-4.10.1.jar:lib/lucene-expressions-4.10.1.jar:lib/lucene-queries-4.10.1.jar:lib/lucene-facet-4.10.1.jar:lib/lucene-queryparser-4.10.1.jar:lib/commons-lang3-3.3.2.jar:lib/commons-logging-1.1.3.jar:lib/httpclient-4.3.5.jar:lib/httpcore-4.3.2.jar:lib/httpasyncclient-4.0.2.jar:lib/httpcore-nio-4.3.2.jar:lib/commons-codec-1.9.jar:lib/commons-io-2.4.jar:lib/tika-app-1.6.jar:bin:data com.hourglassapps.cpi_ii.report.MainReporter <CONDUCTUS_XML_EXPORT> <NUM_BATCHES>
