...
The above command takes as input the stemmed (term -> unstemmed term) mappings of 3grams.dat (mentioned above) and the Lucene index of unstemmed trigrams (./unstemmed_to_stemmed_index). The name of the index is currently hard-coded.
...
Later For all the sorted queries:
    ...
    Once the query's top results are all downloaded the ExpansionDistributer instance notifies any waiting clients (in our code this will be a single IndexingThread instance) of the newly saved documents that require indexing.
...
try {
    mThrower.throwCaught(SomeException.class) //If mThrower has caught an exception, it will be thrown now if it's an instance of the class passed as an argument to throwCaught()
} catch(Throwable t) {
...
A Lucene query is created from the Line instance and the Lucene index that was created while downloading URLs from Bing's result is searched. The following results data is saved to the links.js file for each document in the order of document's rank in the results list:
document title,
path to local copy the document and
a list of absolute start-end character offsets corresponding to query phrase matches within the document.
-------------------------------------------------------
...
n: document rank in results list -- the results list can be found in the links.js file in the above directory.
-------------------------------------------------------
